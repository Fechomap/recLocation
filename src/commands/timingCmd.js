...
module.exports = bot => {
  return async msg => {
    ...
    logger.debug(
      `Grupos registrados: ${JSON.stringify(storage.getAllGroupChats())}`
    );
    logger.debug(
      `Ubicaciones registradas: ${JSON.stringify(storage.getAllUserLocations())}`
    );
    logger.debug('===================');
        ...
        const listener = replyMsg => {
          ...
          if (
            ...
            if (!coordPattern.test(text)) {
              bot.sendMessage(
                chatId,
                '❌ Formato de coordenadas no reconocido. Por favor, ingresa las coordenadas en el formato "latitud,longitud"'
              );
            ...
            if (
              ...
              bot.sendMessage(
                chatId,
                '❌ Las coordenadas ingresadas están fuera de los rangos válidos.'
              );
              bot.removeListener('message', listener);
              resolve(null);
              return;
    ...
    try {
      bot.sendMessage(
        chatId,
        '🔄 Por favor, proporciona las coordenadas de destino...'
      );
      ...
      if (chatId.toString() !== config.ADMIN_GROUP_ID.toString()) {
        bot.sendMessage(
          chatId,
          '✅ El reporte de timing ha sido enviado al Grupo Administrador.'
        );
      }

      logger.info('Reporte de timing ordenado generado y enviado exitosamente');
    } catch (error) {
      logger.error('Error general en comando timing:', {
        error: error.message,
        stack: error.stack
      });
      bot.sendMessage(
        chatId,
        '❌ Error al generar el reporte. Por favor, inténtalo más tarde.'
      );
    }
  };
};
