...
function startApiServer(bot) {
  ...
  router.post('/api/timing', authenticateApiRequest, async (req, res) => {
    try {
      ...
      logger.info(`Solicitud de timing recibida para destino: ${coordinates}, enviando a: ${chatId}`);
      ...
      logger.info(`Grupos disponibles: ${Object.keys(groupChats).length}, Ubicaciones: ${Object.keys(userLocations).length}`);
      
      // Verificar si hay datos
      if (Object.keys(groupChats).length === 0 || Object.keys(userLocations).length === 0) {
        logger.warn('No hay grupos o ubicaciones registradas');
        
        await bot.sendMessage(chatId, '❌ No hay unidades con ubicaciones registradas para calcular tiempos de llegada.', {
          parse_mode: 'Markdown'
        });
      ...
      for (const [groupId, groupName] of Object.entries(groupChats)) {
        ...
        for (const [userId, loc] of Object.entries(users)) {
          try {
            ...
            logger.info(`Ruta calculada para usuario ${storage.getUserName(userId)}: ${route.length/1000}km, ${Math.round(route.duration/60)}min`);
          } catch (error) {
            logger.error(`Error calculando ruta para usuario ${userId}: ${error.message}`);
            reportData[groupName].push({
              userName: storage.getUserName(userId),
      ...
      logger.error('Error en API de timing:', { error: error.message, stack: error.stack });
