...
async function startBot() {
  try {
    ...
    logger.info(
      `Entorno: ${config.IS_PRODUCTION ? 'Producción' : 'Desarrollo'}`
    );
    ...
    bot.onText(/\/test/, msg => {
      logger.info(
        `PRUEBA: Comando /test recibido de ${msg.from.id} en chat ${msg.chat.id}`
      );
      bot.sendMessage(
        msg.chat.id,
        '✅ La prueba de comando funcionó correctamente'
      );
    });
